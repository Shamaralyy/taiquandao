<template>
	<view>
		<view class="content">
			...
			<image class='background' style="z-index: 1;" src="http://39.100.78.225:8002/background.png"></image>
			<!--页面其它部分-->
		...
		<view class="index-btns">
			<view class="btn" @tap="changeSel(index)" v-for="(item,index) in typeList" :key="index"
				:style="index===selindex?'background-color:#c5c5c5':''">
				<view class="text" :style="index===selindex?'color:#050505':''">{{item.typename}}</view>
			</view>
		</view>
		<!-- 比赛规则页面 -->
		<view wx:key="index" v-if="0===selindex">
			<view class="first">
				<view class="recongle"></view>
				<view class="title" style="color: red;">请详细阅读比赛规则和比赛公告</view>
				<view class="text" v-for="(item,index) in rules" :key="index">{{item}}</view>
			</view>
			<view class="first">
				<view class="recongle"></view>
				<view class="title">以下为公告内容</view>
				<view class="text" v-for="(item,index) in announcements" :key="index" style="margin-right: 30rpx;">
					{{item}}
				</view>
			</view>
		</view>
		...
		<view wx:key="index" v-if="1===selindex">
			<view class="thrid">
				<view style="position: relative;padding-top: 20rpx;padding-left: 40rpx;display: flex;">
					<view class="recongle" style="background-color: black;"></view>
					<view class="top">比赛方式</view>
				</view>
				<picker class="msg_picker" mode="selector" :range="selectData.prefsArr" v-if="typeP===1"
					:value="selectData.prefsIndex" @change="pickerChangeGR">
					<view>
						{{selectData.prefsArr[selectData.prefsIndex]}}
					</view>
					<div class="h"></div>
				</picker>
				<picker class="msg_picker" mode="selector" :range="selectData2.prefsArr" v-if="typeP===2"
					:value="selectData2.prefsIndex" @change="pickerChangeJL">
					<view>
		...
		<view wx:key="index" v-if="2===selindex">
			<view v-if="typeP===1" style="display: flex;justify-content: center;margin-top:140rpx">
				<!-- 个人 -->
				<view class="stu_box avatar" style="width: 350rpx;height:400rpx" @click="enrollStu(null,null)">
					<image :src="userInfo.photo" class="avatar" style="z-index: 1;width: 200rpx;height:200rpx"></image>
					<view class="name" style="font-size: 50rpx;">{{userInfo.name}}</view>
					<view class="name" v-show="selectData.prefsIndex==0&&infoP.weight>0"
						style="margin-top: 20rpx;font-size: 35rpx">
						{{infoP.weight +'kg'}}
					</view>
					<view class="name" v-show="selectData.prefsIndex==1" style="margin-top: 10rpx">
			...
			<view v-if="typeP===2">
				<!-- 教练 -->
				<input placeholder="请输入学生姓名..." v-model="stuVal" class="stu_ipt" @input="filterStu" />
				<view class="second">
					<scroll-view>
						<view :class="['stu_box',item.state?'active':'']" v-for="(item,index) in stuList" :key="index"
							@click="enrollStu(item,index)">
							<image style="z-index: 1;" :src="item.avatarUrl" class="avatar"></image>
							<view class="name">{{item.name}}</view>
							<view class="name"
								v-show="(selectData2.prefsIndex==0||selectData2.prefsIndex==2)&&item.weight!=null">
								{{item.weight+'kg'}}
							</view>
							<view class="name" v-show="selectData2.prefsIndex==1||selectData2.prefsIndex==3">
			...
			<button class="signUpBtn" @click="signUp(false)" style="margin-top: 60rpx;">报名</button>
		</view>
		<!-- 费用缴纳页面 -->
			...
			<view class="fourth">
				...
				<view v-else class="box" v-for="(item,index) in enrollList" :key="index">
					...
					<view style="display: flex;">
						<view class="recongle"></view>
						<view style="position: relative;padding-top: 25rpx;padding-left: 50rpx;">
							<image style="z-index: 1;" :src="item.avatarUrl" class="a"></image>
						</view>
						<view class="text"
							v-if="(typeP===1&&selectData.prefsIndex==0)||(typeP===2&&(selectData2.prefsIndex==0||selectData2.prefsIndex==2))">
							体重：{{item.weight}}kg</view>
						<view>
							<view class="text"
								v-if="(typeP===1&&selectData.prefsIndex==1)||(typeP===2&&(selectData2.prefsIndex==1||selectData2.prefsIndex==3))">
								首选：{{item.first}}</view>
							<view class="text"
								v-if="selectData.prefsIndex==1||selectData2.prefsIndex==1||selectData2.prefsIndex==3"
								style="padding-top:12rpx">备选：{{item.second}}</view>
						</view>
						<view class="text" style="margin-left:50rpx">{{item.state}}</view>
						<view class="text" @tap="cancel(item,index)">取消报名</view>
					</view>
				</view>
			</view>
			<button class="mini-btn payBtn" type="primary" @tap="pay">点击缴费</button>
		</view>
	</view>

	<uni-popup ref="popup">
		<uni-popup-dialog mode="input" type="number" :title="'填写体重（'+stuName+'）'" placeholder="请输入(kg) (保留一位小数)"
			@confirm="pswConfirm">
		</uni-popup-dialog>
	</uni-popup>
	...
	<uni-popup ref="popup2">
		<view>
			<view style="color: white;margin-bottom: 30rpx;">首选项目
				<picker class="msg_picker" mode="selector" :range="firstPro.arr" style="background-color: white;"
					:value="firstPro.index" @change="propickerChangeGR">
			...
			<view style="color: white;">备选项目
				<picker class="msg_picker" mode="selector" :range="secondPro.arr" style="background-color: white;"
					:value="secondPro.index" @change="proPickerChange2">
					<view style="color: black;">
...
<script setup>
	...
	function signUp(flag) {
		if (flag) { //在比赛方式页面点击确认
			...
			goodsId.value = [Number(selectData.prefsIndex) + 1, Number(selectData.alternate1Index) + 1, Number(selectData
				.alternate2Index) + 1]
		} else { //点击报名
			...
			if (typeP.value === 1) {
				...
				} else if (selectData.prefsIndex == 1) { //品势
					typeC.value = 2;
					objArr.value = [];
					objArr.value.push({
						id: uni.getStorageSync('openid'),
				...
				typeC.value = 2;
				if (selectData2.prefsIndex == 0 || selectData2.prefsIndex == 2) { //竞技
					const selectedStu = sourceStuList.value.filter(item => item.state === true);
					objArr.value = [];
				...
				} else if (selectData2.prefsIndex == 1 || selectData2.prefsIndex == 3) { //品势
					typeC.value = 2;
					objArr.value = [];
					selectedStu.map(item => {
						objArr.value.push({
	...
	onLoad((options) => {
		...
		if (uni.getStorageSync('enrollObj')) {
			...
			uni.getStorageSync('enrollObj').data.announcement.split('\n').forEach((item, index) => {
				announcements.value.push(item);
			});
			uni.getStorageSync('enrollObj').data.rule.split('\n').forEach((item, index) => {
