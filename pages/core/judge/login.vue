<template>
	<view class="judge_login">
		<!-- 顶部背景 -->
		<image class='background' src="http://39.100.78.225:8002/background.png"></image>

		<!-- 登录、注册 -->
		<view class="center-box">
			<view class="nav">
				<view :class="['left',data.current==1?'select':'']" @tap="click" data-code="1">
					<text>登录</text>
				</view>
			</view>
			<!-- 登录 -->
			<view class="input-box" v-if="data.current==1">
				<view class="wei-input">
					...
					<input class="input" auto-focus placeholder="请输入手机号/登录名" v-model="log.account" />
				</view>
				<view class="wei-input">
					<icon type="success" color="#44ADFB" size="16"></icon>
					<input class="input" auto-focus placeholder="请输入登录密码" v-model="log.password" />
				</view>
				<view class="forget">
					<text>忘记密码?</text>
				</view>
			...
			<view class="sumbit-btn">
				<button class="button" style="background-color: #26221f;font-size: 30rpx;" bindtap="functiona"
					v-if="data.current==1" type="primary" @tap="login">
