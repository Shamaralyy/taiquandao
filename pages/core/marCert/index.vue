      ...
      <view class="id-box" style="border-bottom: 0">
        ...
        <view style="display: flex">
          <view class="id-img" @click="last_upload">
            <image
              src="http://39.100.78.225:8002/yyzz.png"
              mode="aspecFill"
            ></image>
          </view>
        </view>
...
<script setup>
import { guid, testReg } from "@/utils/utils.js";
...
async function submit() {
  ...
  if (
    ...
    if (
      testReg("name", msg.name) &&
      testReg("IDcard", msg.IDcard) &&
      testReg("phone", msg.phone)
    ) {
      const obj = {
        id: String(
          store.state.userInfo.openid.substring(
            store.state.userInfo.openid.length - 6
          )
        ),
      ...
      try {
        let res = await getOrgsInfoAPI(obj);
        console.log(res);
      } catch (e) {}
