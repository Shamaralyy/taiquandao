        ...
        <view class="participants">
          ...
          <view class="qualification">
            <radio-group name="sex" @change="radioChange">
              <label> <radio value="1" /><text>是</text> </label>
    ...
    <label class="check">
      <label>
        <checkbox
          ...
          color="green"
        /> </label
      >我已同意并仔细阅读<a class="agreement" style="display: inline" href="#"
        >《用户隐私协议》</a
      >同意书
    </label>
    <button class="btn_check" type="warn" @tap="postPersonalData">
      完善个人信息
    </button>
  </view>
</template>
...
function postPersonalData() {
  ...
  if (
    baseAvatar.value === "" ||
    userMsg.value.name === "" ||
    userMsg.value.marName === "" ||
    userMsg.value.IDcard === "" ||
    userMsg.value.sex === "未知" ||
    userMsg.value.date === "请选择出生日期" ||
    userMsg.value.isDeclared === "" ||
    avatarName.value === ""
  ) {
    ...
    // 校验身份证：
    if (
      testReg("name", userMsg.value.name) &&
      testReg("IDcard", userMsg.value.IDcard)
    ) {
