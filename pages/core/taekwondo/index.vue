<template>
	<view>
		<view class="race_tab_list">
			<view class="race_tab">
				<text @tap="n = 1" :style="{color:n==1?'gray':''}">冠军赛</text>
			</view>
			<view class="race_tab">
				<text @tap="n = 2" :style="{color:n==2?'gray':''}">新手赛</text>
			</view>
			<view class="race_tab">
				<text @tap="n = 3" :style="{color:n==3?'gray':''}">对决赛</text>
			</view>
		</view>

		<scroll-view @scrolltolower="handleToLower" scroll-y="" v-show="n == 1">
			<view class="race_item" @tap="toDetail(item)" v-for="item in gjs" :key="item">
				<image :src="item.logo" />
				<view class="race_item_text">
					<text class="race_title">{{item.name}}</text><br />
					<div class="race_text_area">
						...
						<text style="color: green;font-size: 25rpx;" v-if="item.remainingDays>0">(正在报名)</text>
		...
		<scroll-view @scrolltolower="handleToLower" scroll-y="" v-show="n == 2">
			<view class="race_item" @tap="toDetail(item)" v-for="item in xss" :key="item.id">
				<image :src="item.logo" />
				<view class="race_item_text">
					<text class="race_title">{{ item.name }}</text><br />
					<div class="race_text_area">
		...
		<scroll-view @scrolltolower="handleToLower" scroll-y="" v-show="n == 3">
			<view class="race_item" @tap="toDetail(item)" v-for="item in djs" :key="item.id">
				<image :src="item.logo" />
				<view class="race_item_text">
					<text class="race_title">{{ item.name }}</text><br />
					<div class="race_text_area">
